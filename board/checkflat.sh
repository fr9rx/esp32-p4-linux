...
# 4096 << 10 -- order 10, the kernel default.
#
# LOWERED FROM 8388608 when this file came across from the hypervisor tree.
# That tree carries a source patch adding CONFIG_ARCH_FORCE_MAX_ORDER to
# arch/riscv/Kconfig and sets it to 11; this tree does not -- the why2025
# 6.18.35 series has no such patch and config/kernel.config has no
# ARCH_FORCE_MAX_ORDER, so MAX_PAGE_ORDER is the default 10 and the real
# per-exec ceiling is 4 MB.
#
# Leaving it at 8 MB would have been worse than having no check at all: a
# binary between 4 and 8 MB would pass the build cleanly and then fail at
# exec on the board, which is exactly the invisible failure this script
# exists to prevent.
#
# Note that why2025 patch 0010's userspace pool does NOT raise this. It
# backs mmap-anon through do_mmap_private; program text/data still comes
# from the buddy allocator as one contiguous block.
LIMIT=4194304
